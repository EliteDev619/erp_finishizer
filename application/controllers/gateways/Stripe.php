...
{
    protected $subscriptionMetaKey = 'perp-subscription-hash';
                ...
                if ($session->payment_intent) {
                    ...
                    if (isset($payment->metadata->InvoiceId)) {
                        ...
                        $invoice = $this->invoices_model->get(
                            ...
                            // 1 Stripe account, multiple ERP installations
    ...
    protected function invoicePaymentSucceededEvent($event)
    {
        ...
        $erpSubscriptionItem = null;

        // Let's check if it's really subscription created from ERP
        foreach ($invoice->lines->data as $item) {
            if (isset($item->metadata[$this->subscriptionMetaKey])) {
                $erpSubscriptionItem = $item;

                break;
            }
        }

        if (!is_null($erpSubscriptionItem)) {
            $dbSubscription = $this->subscriptions_model->get_by_hash($erpSubscriptionItem->metadata[$this->subscriptionMetaKey]);
            ...
            if ($dbSubscription) {
                ...
                $this->subscriptions_model->update($dbSubscription->id, ['next_billing_cycle' => $erpSubscriptionItem->period->end]);
                $this->load->model('payments_model');
